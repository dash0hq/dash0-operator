...
var kubectlEnvPassThrough = []string{
	...
	// Honored by the HTTP client of client-go for the connection to the API server.
	"HTTP_PROXY", "HTTPS_PROXY", "NO_PROXY",
	"http_proxy", "https_proxy", "no_proxy",
	// Honored by crypto/x509 when the system trust store is consulted, e.g. for the certificate of a proxy.
	"SSL_CERT_FILE", "SSL_CERT_DIR",
	// Resolves the kubectl binary itself and whatever kubectl looks up by name.
	"PATH",
}
...
// kubectlEnv returns the environment for the kubectl subprocess. It is assembled from kubectlEnvPassThrough rather
// than inherited from this process, so that the Dash0 auth token this workload holds is not in the environment of a
// subprocess that has no use for it.
//
// By default, kubectl writes its discovery and HTTP caches to $HOME/.kube/cache, which is on the read-only root
// filesystem in this pod. kubectlTmpDir (resolved once at startup, see resolveKubectlTmpDir) points to a writable
// directory, so kubectl's HOME is redirected there to let caching work.
func kubectlEnv(kubectlTmpDir string) []string {
	env := make([]string, 0, len(kubectlEnvPassThrough)+1)
	for _, name := range kubectlEnvPassThrough {
		if value, isSet := os.LookupEnv(name); isSet {
			env = append(env, name+"="+value)
		}
	}
	return append(env, "HOME="+kubectlTmpDir)
}
