...
export fn getenv(nameZ: null_terminated_string) ?null_terminated_string {
    ...
    const envMap = getEnvMap(allocator) catch |err| {
        std.debug.print(dash0_log_prefix ++ "Cannot retrieve process environment, returning null for environment variable '{s}': {}\n", .{ name, err });
        return null;
    };

    return getEnvValue(allocator, envMap, name);
}

fn getEnvValue(allocator: std.mem.Allocator, envMap: std.process.EnvMap, name: [:0]const u8) ?null_terminated_string {
    const value = envMap.get(name);

    if (std.mem.eql(u8, name, "OTEL_RESOURCE_ATTRIBUTES")) {
        const resource_attributes = getResourceAttributes(allocator, envMap) catch |err| {
        ...
        // $1 ==> "A"
        if (value) |val| {
            const resourceAttributes = getResourceAttributes(allocator, envMap) catch |err| {
...
// Instead, we assume the 128KB limit of Linux before 2.6.13, and align it with the memory page
// size of 4KB that the GeneralPurposeAllocator is anyhow going to get.
// Note: Linux mem pages are not always 4KB: https://github.com/ziglang/zig/issues/11308
const memory_page_size = 4096;
const default_memory_limit = 32 * memory_page_size;
...
fn getAllocator() std.mem.Allocator {
    ...
    gpa.requested_memory_limit = getAllocatorMemoryLimit();
    const allocator = gpa.allocator();
    _allocator = allocator;
    return allocator;
}

fn getAllocatorMemoryLimit() usize {
    return getEnvironBufferSize() catch |err| {
        std.debug.print(dash0_log_prefix ++ "Using default memory limit of {} bytes because an error occurred while retrieving the process environment size: {}\n", .{ default_memory_limit, err });
        return default_memory_limit;
    };
}

fn getEnvironBufferSize() !usize {
    var file = try std.fs.cwd().openFile("/proc/self/environ", .{ .mode = .read_only });
    defer file.close();

    // Cannot use file.stat(), it returns zero because we are reading a virtual file
    var buf_reader = std.io.bufferedReader(file.reader());
    var in_stream = buf_reader.reader();

    var size: usize = 0;
    while (true) {
        _ = in_stream.readByte() catch |err| {
    ...
    var res: usize = 0;
    while (size > res) {
        // We could use casting and other dangerous stuff and be a little faster.
        // Or we could do this one-time thing in the dumb and safe way.
        res += memory_page_size;
    }

    // Add another memory page for good measure
    return res + memory_page_size;
}
...
fn getResourceAttributes(allocator: std.mem.Allocator, envMap: std.process.EnvMap) ![]u8 {
...
fn getEnvMap(allocator: std.mem.Allocator) !std.process.EnvMap {
    if (_environ) |envMap| {
        return envMap;
    }
    ...
    const environ_bytes = try in_stream.readAllAlloc(allocator, max_size);

    var res = std.process.EnvMap.init(allocator);

    var i: usize = 0;

    while (environ_bytes.len > i) {
        const env_var: []u8 = std.mem.sliceTo(environ_bytes[i..], 0);
