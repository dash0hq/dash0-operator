...
if [[ ${1:-} == manual-testing ]]; then
  ...
  if [[ -n ${DASH0_AUTHORIZATION_TOKEN:-} ]] && [[ -n ${DASH0_OTEL_EXPORTER_OTLP_ENDPOINT:-} ]] then
      echo "Using DASH0_AUTHORIZATION_TOKEN and DASH0_OTEL_EXPORTER_OTLP_ENDPOINT to render the template test-resources/collector/manual.values.yaml.template. The rendered file test-resources/collector/manual.values.yaml can be used for manual testing and actually reporting data to Dash0."
    ...
    if [[ test-resources/collector/manual.values.yaml.template -nt test-resources/collector/manual.values.yaml ]]; then
      echo "The file test-resources/collector/manual.values.yaml exists, but the template file test-resources/collector/manual.values.yaml.template is newer, indicating it has been changed after the last time the template has been rendered. Refusing to run with an outdated manual.values.yaml file. Also, DASH0_AUTHORIZATION_TOKEN or DASH0_OTEL_EXPORTER_OTLP_ENDPOINT are not set (or both). Both environment variables are required to regenerate manual.values.yaml for the OpenTelemetry collector. Please set both environment variables and try again."
      exit 1
    else
      echo "The file test-resources/collector/manual.values.yaml already exists and seems up to date, the file will be reused. Set DASH0_AUTHORIZATION_TOKEN and DASH0_OTEL_EXPORTER_OTLP_ENDPOINT to generate a new file."
    fi
  else
