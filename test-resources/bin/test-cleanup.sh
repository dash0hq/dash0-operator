...
helm uninstall --namespace "$target_namespace" test-app-ruby --ignore-not-found || true
helm uninstall --namespace test-namespace-2 test-app-ruby-ns2 --ignore-not-found || true
helm uninstall --namespace test-namespace-3 test-app-ruby-ns3 --ignore-not-found || true
...
if kubectl delete -n "$target_namespace" -f test-resources/customresources/dash0slo/dash0slo.yaml; then
  wait_for_third_party_resource_deletion="true"
fi
...
if kubectl delete -n "$target_namespace" -f test-resources/customresources/dash0timeseriesaggregation/dash0timeseriesaggregation.yaml; then
  wait_for_third_party_resource_deletion="true"
fi
...
for team_file in test-resources/customresources/dash0team/*.yaml; do
  if kubectl delete -n "$target_namespace" -f "$team_file"; then
    wait_for_third_party_resource_deletion="true"
  fi
done
