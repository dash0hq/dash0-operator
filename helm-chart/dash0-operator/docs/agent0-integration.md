...
The agent0-connector uses a dedicated service account with a restriced ClusterRole.
The restricted permissions that it grant apply to every namespace.
The agent0-connector is not restricted to the namespaces with a Dash0Monitoring resource.
...
> **Important:** Allowing access to additional resources can have security implications.
When you set up custom rules here, it becomes your responsibility to make sure they still match your security needs.
Only grant access to additional resources types when you are confident that it cannot be used to access sensitive data
(auth tokens, credentials, API secrets, PII etc.) in the cluster.
Failure to do so makes that sensitive data accessible to anyone who has access to your Dash0 organization.
If in doubt, prefer to leave the default rules in place.
