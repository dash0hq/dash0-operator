...
// Test helper for collector_configs_validation_test.go and collector_config_template_coverage_test. This file does not
// contain any tests, it only provides test helpers for other tests.
//
// The collector configuration matrix is the set of collector configurations that are rendered and then handed to the
// collector binary for validation, see collector_configs_validation_test.go. Its purpose is to detect if any of the
// collector configurations the operator might produce uses a configuration setting that the OpenTelemetry collector
// has removed. Such a change makes the collectors crash on startup, and it only surfaces for the configurations that
// actually render the affected setting, which are often the non-default ones.
//
// The matrix consists of a minimal (every setting off) and a maximal (all settings enabled) baseline; plus, for every
// setting that a collector configuration template branches on, one entry that flips exactly that setting against both
// baselines.
//
// collector_configs_template_coverage_test.go verifies that the matrix includes both sides of every template
// conditional, so a new conditional cannot be added without extending the matrix.

// collectorConfigMatrixEntry is one configuration of the config matrix. It includes everything that is needed to render
// and validate the collector configuration templates for a specific combination.
...
// renderedCollectorConfig represents a rendered collector configuration together with the name of its matrix entry and
// the collector type it belongs to.
...
// matrixSetting is one configuration option that at least one collector configuration template branches on. The matrix
// contains one entry per setting and baseline, with only this setting flipped relative to the baseline.
...
// (for example a Dash0 exporter versus a pure pass-through exporter) are covered by the per-setting entries instead.
...
// matrixFilters lists every signal and object type the filter processor can be rendered for.
...
// matrixTransforms lists every signal the transform processor can be rendered for.
...
// matrixSettings lists the configuration options that the collector configuration templates branch on. Every setting is
// flipped in isolation against both baselines.
func matrixSettings() []matrixSetting {
...
// enableSignalControl brings an entry into a state where Signal Control is active, so that a setting below Signal
...
// collectorConfigMatrix returns both baselines plus, for every setting, the baseline with only that setting flipped.
func collectorConfigMatrix() []collectorConfigMatrixEntry {
	baselines := []struct {
		entry   collectorConfigMatrixEntry
		flipsTo bool
	}{
		// The minimal baseline has everything switched off, so flipping a setting means enabling it, and vice versa for
		// the maximal baseline.
		{entry: minimalMatrixBaseline(), flipsTo: true},
		{entry: maximalMatrixBaseline(), flipsTo: false},
	}

	matrix := make([]collectorConfigMatrixEntry, 0, 2+2*len(matrixSettings()))
	for _, baseline := range baselines {
		matrix = append(matrix, baseline.entry)
	}
	for _, baseline := range baselines {
		for _, setting := range matrixSettings() {
			entry := baseline.entry
			setting.toggle(&entry, baseline.flipsTo)
			entry.name = fmt.Sprintf("%s__%s-%s", baseline.entry.name, setting.name, enabledSuffix(baseline.flipsTo))
