...
var _ = Describe("Create an operator configuration resource at startup", Ordered, func() {
	...
	It("should fail validation if no endpoint has been provided", func() {
		...
		_, err := autoOperatorConfigurationResourceHandler.CreateOrUpdateOperatorConfigurationResource(ctx, &OperatorConfigurationValues{
			Token: AuthorizationTokenTest,
		}, &logger)
		Expect(err).To(
			MatchError(
	...
	It("should create a new operator configuration resource with a token", func() {
		autoOperatorConfigurationResourceHandler.NotifiyOperatorManagerJustBecameLeader(ctx, &logger)
		_, err := autoOperatorConfigurationResourceHandler.CreateOrUpdateOperatorConfigurationResource(ctx, &operatorConfigurationValuesWithToken, &logger)
	...
	It("should create a new operator configuration resource with a secret reference", func() {
		autoOperatorConfigurationResourceHandler.NotifiyOperatorManagerJustBecameLeader(ctx, &logger)
		_, err := autoOperatorConfigurationResourceHandler.CreateOrUpdateOperatorConfigurationResource(ctx, &operatorConfigurationValuesWithSecretRef, &logger)
		Expect(err).ToNot(HaveOccurred())
	...
	It("should wait for the replica to become leader", func() {
		_, err :=
	...
	It("should wait for the webhook service endpoint ready check", func() {
		autoOperatorConfigurationResourceHandler.NotifiyOperatorManagerJustBecameLeader(ctx, &logger)
	...
	It("should update the existing resource if there already is an auto-operator-configuration-resource", func() {
		...
		_, err = autoOperatorConfigurationResourceHandler.CreateOrUpdateOperatorConfigurationResource(ctx,
