...
//  2. a change event on one of the target-allocator related resources that the operator manages
//
...
func (m *TargetAllocatorManager) ReconcileTargetAllocator(
	...
	if m.resourcesHaveBeenDeletedByOperator.Load() {
		...
		case TriggeredByWatchEvent:
			if m.developmentMode {
				logger.Info("OpenTelemetry target-allocator resources have already been deleted, ignoring reconciliation request.")
			}
			return false, nil
	...
	operatorConfigurationResource, err := resources.FindOperatorConfigurationResource(m.Client, ctx, &logger)
	// todo: revisit what should happen if the operatorConfigurationResource is nil
	if operatorConfigurationResource == nil || err != nil {
		return false, err
	}
...
func (m *TargetAllocatorManager) createOrUpdateTargetAllocator(
	...
	resourcesHaveBeenCreated, resourcesHaveBeenUpdated, err := m.targetAllocatorResourceManager.CreateOrUpdateTargetAllocatorResources(ctx, operatorConfigurationResource, logger)
