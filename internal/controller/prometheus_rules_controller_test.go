...
const (
	thresholdAnnotationsMissingMessagePattern = "The check rule %s/%s uses the token $__threshold in its expression, but has neither the threshold-degraded nor the threshold-critical annotation."
)
...
var _ = Describe("The Prometheus rule controller", Ordered, func() {
	...
	Describe("the Prometheus rule resource reconciler", func() {
					...
					Groups: []prometheusv1.RuleGroup{
						{
							...
							Rules: []prometheusv1.Rule{
								...
								{
									Alert: "rule_1_2", // PUT requests will receive HTTP 500
						...
						},
						{
							...
							Rules: []prometheusv1.Rule{
								...
								{
									Alert: "rule_2_3", // PUT requests will receive HTTP 500
									Expr:  intstr.FromString("vector(1)"),
								},
							},
						},
			...
			verifyPrometheusRuleSynchronizationResultHasBeenWrittenToMonitoringResourceStatus(
				...
				dash0v1alpha1.PrometheusRuleSynchronizationResult{
					...
					SynchronizationErrors: map[string]string{
						"group_1 - rule_1_2": "^unexpected status code 500 when updating/creating/deleting the rule \"group_1 - rule_1_2\" at https://api.dash0.com/api/alerting/check-rules/dash0-operator_.*_test-dataset_test-namespace_test-rule_group_1_1\\?dataset=test-dataset, response body is {}\n$",
						"group_2 - rule_2_3": "^unexpected status code 500 when updating/creating/deleting the rule \"group_2 - rule_2_3\" at https://api.dash0.com/api/alerting/check-rules/dash0-operator_.*_test-dataset_test-namespace_test-rule_group_2_2\\?dataset=test-dataset, response body is {}\n$"},
					InvalidRulesTotal: 3,
					InvalidRules: map[string][]string{
						"group_1 - rule_1_1": {thresholdAnnotationsMissingMessage("group_1", "rule_1_1")},
						"group_1 - 4":        {"rule has neither the alert nor the record attribute"},
						"group_2 - rule_2_2": {thresholdAnnotationsMissingMessage("group_2", "rule_2_2")},
					},
				},
			)
		})
		...
		It("reports as failed if no Prometheus rule is synchronized succcessul", func() {
			...
			for _, pathRegex := range []string{
				"dash0-operator_.*_test-dataset_test-namespace_test-rule_group_1_1",
				"dash0-operator_.*_test-dataset_test-namespace_test-rule_group_2_1",
			} {
				gock.New(ApiEndpointTest).
					Put(fmt.Sprintf("%s.*%s", checkRuleApiBasePath, pathRegex)).
					MatchParam("dataset", DatasetTest).
			...
			verifyPrometheusRuleSynchronizationResultHasBeenWrittenToMonitoringResourceStatus(
				...
				dash0v1alpha1.PrometheusRuleSynchronizationResult{
					...
					InvalidRules: map[string][]string{
						...
						"group_2 - rule_2_1": {thresholdAnnotationsMissingMessage("group_2", "rule_2_1")},
					},
				},
			)
		})
			...
			func(config thresholdValidationTestConfig) {
				...
				Entry(
					...
					thresholdValidationTestConfig{
						annotations:              nil,
						expectedValidationIssues: []string{thresholdAnnotationsMissingMessage("group", "alert")},
					}),
				Entry(
				...
				Entry(
					...
					thresholdValidationTestConfig{
						...
						expectedValidationIssues: []string{thresholdAnnotationsDegradedNonNumericalMessage("group", "alert", "1s")},
					}),
				Entry(
					...
					thresholdValidationTestConfig{
						...
						expectedValidationIssues: []string{thresholdAnnotationsCriticalNonNumericalMessage("group", "alert", "abc")},
					}),
				Entry(
					...
					thresholdValidationTestConfig{
						...
						expectedValidationIssues: []string{
							thresholdAnnotationsDegradedNonNumericalMessage("group", "alert", "1s"),
							thresholdAnnotationsCriticalNonNumericalMessage("group", "alert", "abc"),
						},
					}),
			})
	})
...
func thresholdAnnotationsMissingMessage(group string, name string) string {
	return fmt.Sprintf(thresholdAnnotationsMissingMessagePattern, group, name)
}

func thresholdAnnotationsDegradedNonNumericalMessage(group string, name string, value string) string {
	return fmt.Sprintf(thresholdAnnotationsNonNumericalMessagePattern, group, name, "degraded", value)
}

func thresholdAnnotationsCriticalNonNumericalMessage(group string, name string, value string) string {
	return fmt.Sprintf(thresholdAnnotationsNonNumericalMessagePattern, group, name, "critical", value)
}
