...
// the operator manager deployment is deployed.
...
// WaitForAllowlistSynchronizerToBecomeReady polls the dash0-allowlist-synchronizer resource until its Ready condition is True.
// If the AllowlistSynchronizer CRD does not exist in the cluster, it returns immediately with success (there is nothing to wait
// for).
