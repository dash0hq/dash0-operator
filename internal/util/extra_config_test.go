...
`)
			...
			Describe("parse config map and convert to resource requirements", func() {
				...
				It("should parse the config map content with all resource values set and convert", func() {
					_, err := tmpFile.WriteString(`
initContainerResources:
  limits:
    cpu: 200m
...
`)
				...
				It("should merge partial config and defaults and convert", func() {
					_, err := tmpFile.WriteString(`
initContainerResources:
  limits:
    cpu: 200m
...
`)
	...
	Describe("extra config watcher", func() {
		var tmpDir string
		var tmpFile *os.File
		var watcher *ExtraConfigWatcher

		BeforeEach(func() {
			var err error
			tmpDir, err = os.MkdirTemp("", "watch-extra-config-test-*")
			Expect(err).ToNot(HaveOccurred())
			tmpFile, err = os.CreateTemp(tmpDir, "extra.yaml")
			Expect(err).ToNot(HaveOccurred())
		})
...
`)
		...
		It("should notify clients when a change is detected", func() {
			watcher := NewExtraConfigWatcher()
			client1 := &DummyExtraConfigClient{}
			watcher.AddClient(client1)
			client2 := &DummyExtraConfigClient{}
...
`)
			...
			Eventually(func(g Gomega) {
				for _, c := range clients {
					g.Expect(c.updateExtraConfigCalls).To(Equal(1))
					updatedConfig := c.updatedConfig
					g.Expect(updatedConfig).ToNot(BeNil())
		...
		It("should debounce file system changes detected", func() {
			watcher = NewExtraConfigWatcher()
...
`)
			...
			Eventually(func(g Gomega) {
				g.Expect(client.updateExtraConfigCalls).To(Equal(1))
				updatedConfig := client.updatedConfig
				g.Expect(updatedConfig).ToNot(BeNil())
				g.Expect(updatedConfig.CollectorFilelogOffsetStorageVolume).ToNot(BeNil())
				filelogOffsetStorageVolume := updatedConfig.CollectorFilelogOffsetStorageVolume
				g.Expect(filelogOffsetStorageVolume).ToNot(BeNil())
...
type DummyExtraConfigClient struct {
	updateExtraConfigCalls int
